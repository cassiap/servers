...
disp_page = disp[to_show].iloc[start:end]
...
st.download_button("Baixar (CSV)", data=df_f.to_csv(index=False).encode("utf-8"),
                   file_name="servidores_filtrado.csv", mime="text/csv")
st.download_button("Baixar (XLSX)", data=to_xlsx_bytes(df_f.rename(columns={c:colmap.get(c,c) for c in df_f.columns})),
                   file_name="servidores_filtrado.xlsx", mime="application/vnd.openxmlformats-officedocument.spreadsheetml.sheet")

st.caption("Dica: cabeçalhos reconhecidos: 'Equipe Responsável', 'Sistema/Serviço/Produto', 'Descrição do IC', 'Nome'/'Hostname', 'Ambiente'.")
